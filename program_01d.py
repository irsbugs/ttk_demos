...
#               Change to Enable / Disable toggle button
#               Using .state method and len()
#
#
...
class GUI_Application_Feature(ttk.Frame):
    ...
    def button_3_callback(self):  # ***
        ...
        if len(disabled) == 1:
            ...
            self.button_3.configure(text="Enable")  # Toggle the button text
        else:
            # Currently disabled so enable
            self.button_1.state(['!disabled'])
            self.button_2.state(['!disabled'])
            self.button_3.configure(text="Disable")  # Toggle the button text
