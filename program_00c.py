#!/usr/bin/env python3
#
# Program:      program_00c.py
#
# Objective:    GUI application template.
#               Standard GUI provides labels and a button
#               Feature GUI - A blank template - Not used.
#               Command line argument 2 or 4 to launch multiple frames
#
...
INFO_1 = ("Display the standard GUI application class, 1, 2 or 4 times.")
INFO_2A = "No command line argument was passed on launching."
INFO_2B = "The command line argument was "
CLOSE_TEXT = "Close"

# Define Variables - Standard Section:
...
class GUI_Application_Feature(ttk.Frame):
    ...
    def widget_callback(self):
        """Callback from a widget"""
        # Add processing code here...
        # Of call another function... E.g Call...
        # self.main_program_inside_GUI_class(status)
        # Or...
        # main_program_outside_GUI_class(self.widget, status):
        pass

    def main_program_inside_GUI_class(self, status):
        """
        Calling to this code in the class saves having to pass widget info.
        """
        pass
...
def main_program_outside_GUI_class(label1, status):
    """
    Calling to this code, which is outside the class, may require having to
    pass widget info.
    """
    pass
...
class GUI_Application_Standard(ttk.Frame):
    ...
    def create_standard_widgets(self, argument):
        ...
        #                           pady=5, sticky="w")
        # Buttons
        self.button_1_standard.grid(row=4, column=2, padx=10, pady=10,
                                    sticky="e")
...
if __name__ == "__main__":
    ...
    if argument_1 == "2":
        main_gui = GUI_Application_Standard(root, argument_1).grid(row=0,
                                                                   column=0)
        main_gui = GUI_Application_Standard(root, argument_1).grid(row=1,
                                                                   column=0)
        root.mainloop()
        sys.exit()

    # Open the four GUI Application class. Use grid to place in different rows
    if argument_1 == "4": 
        main_gui = GUI_Application_Standard(root, argument_1).grid(row=0,
                                                                   column=0)
        main_gui = GUI_Application_Standard(root, argument_1).grid(row=1,
    ...
    # Else just display it once
    main_gui = GUI_Application_Standard(root, argument_1).grid(row=0, column=0)    
    root.mainloop()
