...
INFO_1 = "Display the Progress Bar with style. Indeterminate mode."  # ***
...
BUTTON_1_TEXT = "Start - Constant"
BUTTON_2_TEXT = "Start - Slows"
LABELFRAME_3_TEXT = "Progress bar - Indeterminate"
...
class GUI_Application_Feature(ttk.Frame):
    ...
    def create_feature_widgets(self, argument):
        ...
        # Create Buttons:
        # Button start with constant speed of progress bar
        self.button_1 = ttk.Button(self.label_frame_3, text=BUTTON_1_TEXT,
                                   command=self.button_1_callback)
        # Button start with progress bar slowing down as calculations increase
        self.button_2 = ttk.Button(self.label_frame_3, text=BUTTON_2_TEXT,
        ...
        self.progress_bar_1.grid(row=1, column=0, columnspan=2,
    ...
    def action_on_launch(self, argument):
        ...
        # Pass the command line argument. E.g. millisecs of delay
        self.progress_bar_milliseconds_delay = argument

    # ===== Widget call backs =====
    def button_1_callback(self):  # ***
        """Start calculating primes with constant progress bar"""
        # self.progress_bar_1.configure(maximum=1000)
        self.label_1.config(text="", style='green.TLabel')
        self.progress_bar_int.set(0)  # Start point from 0 to maximum E.g. 100
        self.time_start = time.time()
        self.main_program_inside_GUI_class("constant")

    def button_2_callback(self):  # ***
        """Start calculating primes with slowing down progress bar"""
        self.label_1.config(text="", style='green.TLabel')
        self.progress_bar_int.set(0)  # Start point from 0 to maximum E.g. 100
        self.time_start = time.time()
        self.main_program_inside_GUI_class("slows")

    def main_program_inside_GUI_class(self, speed):
        """Calling to code in the loop saves having to pass widget info"""
        # Create a list of prime numbers from start_integer.
        # Designed to be CPU intensive. Iterates through every number.
        start_integer = self.argument
        prime_number_list = []
        count = 0
        self.get_prime_numbers = True

        string = "Prime Number:"
        bar_counter = 0
        i = start_integer
        while self.get_prime_numbers:
            count = 0
            for j in range(1, i):
                # Progress bar with display with constant speed
                if speed == "constant":
                    bar_counter += 1
                    if bar_counter == 100000:
                        self.progress_bar_1.step(1)
                ...
                if i % j == 0:
                    count += 1
            if count == 1:
                # If applicable add prime number to list and display
                prime_number_list.append(i)
                self.label_1.config(text="{}{}".format(string, i),
                                    style='green.TLabel')
            i += 1

            # Progress bar with display slowing down with each larger integer
            if speed == "slows":
                if i % 20 == 0:
                    self.progress_bar_1.step(1)
                    self.update_idletasks()

            if i == start_integer + 10000:
                """Not threaded code, so need to bring an end to this loop"""
                self.get_prime_numbers = False

        # print("Total Increments of the Progress Bar:{}"
        #       .format(self.progress_bar_int.get()))
        # print(prime_number_list)
        self.progress_bar_1.stop()
        self.time_stop = time.time()

        string = ("Duration: {0:.3f} seconds."
                  .format(self.time_stop - self.time_start))
        string1 = ("Total Increments:{}. Prime Numbers:{}"
                   .format(self.progress_bar_int.get(),
                           len(prime_number_list)))
        # Update the label
        self.label_1.config(text="{} {}".format(string, string1),
                            style='green.TLabel')
...
===
    # Create a list of prime numbers from start_integer.
    # Designed to be CPU intensive. Iterates through every number.
    start_integer = 100000
    integer_range = 100
    prime_list = []
    count = 0
    self.progress_bar_1.start()
    time.sleep(1)
    for i in range(start_integer, start_integer + integer_range):
        #print("i = {}".format(i))
        count = 0
        for j in range(1, i):
            self.update_idletasks()
