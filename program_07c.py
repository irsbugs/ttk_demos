#!/usr/bin/env python3
#
# Program:      program_07c.py
#
# Objective:    GUI application template using buttons and labels
#               Use same overall layout as program_01.py
#               Introduce ttk.Combobox with LabelFrames using style
#               postcommand callback alters drop down list.
#               Command line option not used.
#
...
INFO_1 = ("Combobox widget read-only. Postcommand callback to build list.\n"
          "Command line arguments are unsupported.")
...
class GUI_Application_Feature(ttk.Frame):
    ...
    def create_feature_widgets(self, argument):
        ...
        # Drop down list for combobox. Put 0 in the list to get it started.
        # The postcommand=self.update_combo_list callback will update the list.
        self.combobox_1_list = [0]
        # Set a pass counter for each callkback
        self.combo_pass_count = 0
        ...
        # print("TCombobox layout:\n{}".format(self.style.layout('TCombobox')))
        # [('Combobox.field', {'sticky': 'nswe', 'children':
        # [('Combobox.downarrow', {'side': 'right', 'sticky': 'ns'}),
        # ('Combobox.padding', {'expand': '1', 'children':
        # [('Combobox.textarea', {'sticky': 'nswe'})], 'sticky': 'nswe'})]})]

        # print("TCombobox.textarea:\n{}".format(self.style.element_options(
        # 'TCombobox.textarea')))
        # field ('-fieldbackground', '-borderwidth')
        # downarrow ('-background', '-relief', '-borderwidth', '-arrowcolor',
        #           '-arrowsize')
        # padding ('-padding', '-relief', '-shiftrelief')
        # textarea: ('-font', '-width')

        self.style.configure('x.TCombobox', padding=5,
                             background='light blue', borderwidth=5,
                             arrowsize=25, arrowcolor="green")
        # Font? Style doesn't change font=('FreeSans', 20))
        # Relief? Style doesnt' seem to work on relief="ridge"
        ...
        # Create Label Frame for combobox_1
        self.labelframe_1 = ttk.Labelframe(self, text="Select number 0 to 999")

        # Create Combobox_1  # ***
        # Use register to define functions to validate and process invalid
        # Use postcommand callback to build the list
        self.combobox_1 = ttk.Combobox(
            self.labelframe_1,
            textvariable=self.combobox_1_text,
            height=10,
            width=10,
            justify="center",  # left, right, or center
            validate='focusin',  # all, key, focus, focusin, focusout, or none
            ...
            postcommand=self.update_combo_list,
            style='x.TCombobox',
            font=("FreeSans", 16)  # Font not changing with style
            )
    ...
    def update_combo_list(self):
        """The combo_list is updated each time it is used"""
        if self.combo_pass_count == 3:
            self.combo_pass_count = 0

        if self.combo_pass_count == 0:
            # Select the hundreds.  Make list 000, 100, ...900
            # print(int(self.combobox_1_text.get()))
            current_value = int(self.combobox_1_text.get())
            self.combobox_1_list = []
            for i in range(10):
                self.combobox_1_list.append(str(i * 100))
            self.combobox_1['values'] = self.combobox_1_list

        if self.combo_pass_count == 1:
            # Select the tens. Make list x00, x10, x20, ...x90
            current_value = int(self.combobox_1_text.get())
            self.combobox_1_list = []
            for i in range(10):
                self.combobox_1_list.append(str(current_value + i * 10))
            self.combobox_1['values'] = self.combobox_1_list

        if self.combo_pass_count == 2:
            # Select the units. Make list xy0, xy1, xy2, ...xy9
            current_value = int(self.combobox_1_text.get())
            self.combobox_1_list = []
            for i in range(10):
                self.combobox_1_list.append(str(current_value + i * 1))
            self.combobox_1['values'] = self.combobox_1_list

        # Increment the pass count through this callback
        self.combo_pass_count += 1

    def is_valid_combo_1(self, txt):
        """If pass count is 3 then completed building number so display it."""
        # print(self.combo_pass_count)
        if self.combo_pass_count == 3:
            self.label_1.config(text="{}".format(self.combobox_1_text.get()))
        return True
